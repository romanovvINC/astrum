...
{
    public class ChangeCoverCommandHandler : CommandHandler<ChangeCoverCommand, Result<ChangeCoverResponse>>
    {
        ...
        public ChangeCoverCommandHandler(IUserProfileRepository profileRepository, IFileStorage fileStorage,
        ...
        public override async Task<Result<ChangeCoverResponse>> Handle(ChangeCoverCommand command, CancellationToken cancellationToken = default)
        {
            if (command.Username == null || command.CoverImage == null)
            ...
            using (var binaryReader = new BinaryReader(command.CoverImage.OpenReadStream()))
            {
                imageBytes = binaryReader.ReadBytes((int)command.CoverImage.Length);
            }

            var avatar = new FileForm()
            {
                ContentType = command.CoverImage.ContentType,
                FileName = command.CoverImage.Name,
                FileBytes = imageBytes
            };

            var uploadResult = await _fileStorage.UploadFile(avatar, cancellationToken);
            profile.CoverImageId = uploadResult.UploadedFileId;
            try
            {
                ...
                var response = new ChangeCoverResponse { FileUrl = url };
                return Result.Success(response);
            }
            catch (Exception ex)
            {
                return Result.Error(ex.Message, "Ошибка при изменении обложки.");
            }
        }
    }
}
